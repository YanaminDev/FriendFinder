...
// Update drinking
router.patch('/update/drinking', async (req, res) => {
  try {
    const userId = (req as any).user.sub
    if (!userId) {
        return res.status(400).json({ message: "User ID not found in token" });
...
// Update pet
router.patch('/update/pet', async (req, res) => {
  try {
    const userId = (req as any).user.sub
    if (!userId) {
        return res.status(400).json({ message: "User ID not found in token" });
...
// Update smoke
router.patch('/update/smoke', async (req, res) => {
  try {
    const userId = (req as any).user.sub
    if (!userId) {
        return res.status(400).json({ message: "User ID not found in token" });
...
// Update workout
router.patch('/update/workout', async (req, res) => {
  try {
    const userId = (req as any).user.sub
    if (!userId) {
        return res.status(400).json({ message: "User ID not found in token" });
