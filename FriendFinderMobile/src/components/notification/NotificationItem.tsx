  ...
  <View className="bg-white px-4 py-3.5 border-b border-gray-50 gap-3">
    ...
    <View className="flex-row gap-3 items-start">
      <View className="relative w-[52px] h-[52px]">
        <Image source={{ uri: notification.avatar }} className="w-[52px] h-[52px] rounded-full bg-gray-200" />
        {!notification.isRead && (
    ...
    {notification.hasActions ? (
      <View className="flex-row gap-2.5">
        <Button variant="outline" color="gray" label="Pass" onPress={onPass ?? (() => {})} className="flex-1 h-[42px]" />
        <Button label="Let's Hang Out" onPress={onHangOut ?? (() => {})} className="flex-[1.4] h-[42px]" />
      </View>
    ) : (
      <Button variant="outline" color="gray" label="View Details" onPress={onViewDetails ?? (() => {})} className="h-[42px]" />
    )}
  </View>
);
